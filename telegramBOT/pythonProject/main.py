...
def main(message):
    ...
    bug_fix = types.InlineKeyboardButton("Проблема с сайтом", callback_data='bug_fix')
    markup.row(suppurt_user, bug_fix)
...
def messanger(message):
    ...
    text = message.text
    bot.send_message(sender_chat_id, "Вы перешли в чат с поддержкой.")
